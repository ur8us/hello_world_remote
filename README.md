...
### Connect Local VS Code to `192.168.64.125`

If VS Code is running locally on your PC and the Raspberry Pi is reachable at `192.168.64.125`, set up SSH like this.

Open your SSH config on the PC:
...
3. Select `rpi`
4. Wait for the Remote-SSH window to open on the Raspberry Pi
5. Open `/home/pi/hello_world_remote` in that remote window

If you do not want to create a host alias, you can also connect directly to:

```text
pi@192.168.64.125
```

After the SSH window opens, install the `C/C++` extension in that remote window if VS Code asks for it.

## VS Code Debug Configuration

This project uses a direct `gdb` launch over Remote-SSH.
...
```json
{
  ...
  "configurations": [
    {
      "name": "Debug hello_debug (Remote-SSH)",
      "type": "cppdbg",
      "request": "launch",
      "program": "/home/pi/hello_world_remote/hello_debug",
      "cwd": "/home/pi/hello_world_remote",
...
`F5` now triggers the VS Code task `build debug` automatically before launch, so `hello_debug` is created even in a clean folder.
